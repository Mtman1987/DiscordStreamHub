...
export function TwitchBotLinkingCard({ serverId }: { serverId: string }) {
  const handleLinkBot = () => {
    ...
    const authUrl = `https://id.twitch.tv/oauth2/authorize?client_id=${process.env.NEXT_PUBLIC_TWITCH_CLIENT_ID}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=${encodeURIComponent(scopes)}&state=${serverId}`;
